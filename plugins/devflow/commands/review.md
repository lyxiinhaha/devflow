...
**原逻辑保留性审查（对 diff 中每个有修改的已有函数/方法执行）：**

| 检查项 | 判断方式 | 标记 |
|--------|---------|------|
| 原有 null/空值保护被删除 | 在 diff `+` 行中找不到等价的 null check / 空判断 | 🔴 CRITICAL |
| 原有异常捕获被移除 | 异常未在 diff 涉及的调用链上层处理 | 🔴 CRITICAL |
| 原有返回值语义变化，调用方未同步 | 搜索所有已知调用方文件，未在 diff 中出现 | 🟡 Warning（调用方可能在 diff 外，需确认） |

新增文件（diff 只有 `+` 行、无 `-` 行的文件）跳过此三条。
...
- **Skill**: {workspace.reviewSkills 中命中的 skill | 通用审查}
...
### 6.5 完成门禁检查（APPROVED 后，进入步骤 7 前）

**触发条件（满足任一即触发，否则跳过直接进入步骤 7）：**
- tasks.md 任务数 ≥ 5，或
- open-issues.md 存在且有 status=open/paused 的条目，或
- CodeGraph 影响面涉及 ≥ 2 个非目标模块

触发后读取：tasks.md 各任务 Verification 字段、open-issues.md open/paused 条目、CodeGraph 影响面结果，输出四问并等待回答：

```
⚠️ 完成门禁（全部确认后进入合并步骤）

Q1 验证状态
   · code_done（未本地验证）：{n} 个 → {任务 ID 列表}
   · locally_verified：{m} 个
   · not_started / 字段缺失：{k} 个
   本地运行验证全部通过？[y / 跳过：{原因}]

Q2 未解除假设
   open-issues.md open/paused 条目：{n} 个
   · OI-1：{描述摘要}（{类型}）
   （最多展示 3 条，超出说明总数）
   这些不影响合并？[y / 跳过：{原因}]

Q3 跨模块回归
   CodeGraph 影响面（非目标模块）：{模块名列表}
   这些模块回归验证通过？[y / 跳过：{原因}]

Q4 回归义务
   open-issues.md type=regression 且 status=open 条目：{n} 个
   · {描述摘要}
   全部回归验证完成？[y / 跳过：{原因}]
```

处理规则：
- 全部 `y` → 记录到 review.md，进入步骤 7
- 任何「跳过：原因」→ 将原因追加到 review.md 的 APPROVED 记录下方，进入步骤 7
- 空回车不接受，必须回答
