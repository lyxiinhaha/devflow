...
- **CodeGraph MCP**：已安装时优先通过 `codegraph_explore` / `codegraph_impact` / `codegraph_trace` 查询代码结构；未安装时降级为 Kiro 内置代码索引
- **Meegle / Figma / YApi**：可选，配置方式见 `.devflow/workspace.json`；未配置时对应步骤跳过，不阻断流程
